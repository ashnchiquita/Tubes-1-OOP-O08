...
void CommandHandler<T>::turnCommand(string prompt, bool constraints[3], string abilitytype){
    ...
    if (this->input == string("HELP")){
        throw CommandHelpException();
    }
    else{
        if (constraints[0]){
            if (this->input == abilitytype)
                if (!constraints[1]) throw CommandCardUsedException(abilitytype);
                else if (constraints[2]) throw CommandDisabledException(abilitytype);
                else return;
